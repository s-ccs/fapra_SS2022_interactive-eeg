...
begin
	sidebar = Div([
		...
		---
		Select original signal A $(test = @bind signalA Select(["sin.(T1xs)","cos.(T1xs)"],default="sin.(T1xs)"))\
		Choose the value for T1 $(@bind T1 PlutoUI.Slider(1:1:10;default=1,show_value=true))
		
		---
		
		Select signal original B $(test = @bind signalB Select(["sin.(T2xs)","cos.(T2xs)"],default="cos.(T2xs)"))\
		Choose the value for T2 $(@bind T2 PlutoUI.Slider(1:1:10;default=3,show_value=true))

		---

		""",
		md"""Select mixing parameter a $(@bind a PlutoUI.Slider(0:0.1:10;default=2,show_value=true))""",
		md"""Select mixing parameter b $(@bind b PlutoUI.Slider(0:0.1:10;default=3.7,show_value=true))""",
		md"""Select mixing parameter c $(@bind c PlutoUI.Slider(0:0.1:10;default=4.8,show_value=true))""",
		md"""Select mixing parameter d $(@bind d PlutoUI.Slider(0:0.1:10;default=7.9,show_value=true))""",
		md"---",
...
begin
	...
	if A_label == B_label && T1==T2
		print("WARNING: ICA can't function well, because A and B are identical.")
	end
end
...
begin
	...
	g1b = f1[2,2]
	g1 = f1[2, 1]
	lines(g1, xs, A, color = :skyblue, label = "$A_label($T1 xs）")
	lines!(g1, xs, B, color = :salmon, label = "$B_label($T2 xs）")
	axislegend()
	density(g1a, A, color = :skyblue)
	density(g1b, B, color = :salmon, direction = :y)
	g11 = f1[1, 2]
	scatter(g11, A, B, color = :green, markersize = 4, axis=(xlabel=L"f1(x)", ylabel=L"f2(x)"))
	f1
end
...
begin
	...
	g2a = f2[1,1]
	g2b = f2[2,2]
	lines(g2, xs, line1, color = :skyblue, label = "$a$A_label( $T1 xs）+ $b$B_label")
	lines!(g2, xs, line2, color = :salmon, label = "$c$A_label( $T1 xs）+ $d$B_label")
	axislegend()
	density(g2a, line1, color = :skyblue)
	density(g2b, line2, color = :salmon, direction = :y)
	g22 = f2[1, 2]
	scatter(g22, line1, line2, color = :green, markersize = 4, axis=(; xlabel=L"f1(x)", ylabel=L"f2(x)"))
	f2
end
...
begin
	...
	line1_demean = line1 - fill(mean1, (300,1))
	line2_demean = line2 - fill(mean2, (300,1)) #change
	...
	g3a = f3[1,1]
	g3b = f3[2,2]
	density(g3a, whitening_matrix[1,:], color = :skyblue)
	density(g3b, whitening_matrix[2,:], color = :salmon, direction = :y)
	lines(g3, xs, whitening_matrix[1,:], color = :skyblue, label = "whiten Mixed signal 1")
	lines!(g3, xs, whitening_matrix[2,:], color = :salmon, label = "whiten Mixed signal 2")
	axislegend()
	
	array = transpose(whitening_matrix)
	x1 = array[:,1]
	x2 = array[:,2]

	g33 = f3[1, 2]
	scatter(g33, x1, x2, color = :green, markersize = 4, axis=(; xlabel=L"f1(x)", ylabel=L"f2(x)"))
	f3
end
...
git-tree-sha1 = "79107f717bae377da585be6b61476197d5b543c2"
